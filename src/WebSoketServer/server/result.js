...
class Result{
    constractor(successful,msg){
        this.successful=successful;
        this.msg=msg;
    }
}

export const makeOk = (msg) =>
     new Result("Ok", msg);
    
export const makeFailure = (msg) =>
    new Result("Failure", msg);

export const isOk = (r) =>
    r.tag === "Ok";

export const isFailure = (r) =>
    r.tag === "Failure";

export const bind = (r, f) =>//(Result<U>, (x: T) => Result<U>)=> Result<U>
    isOk(r) ? f(r.value) : r;

export const either = (r, ifOk, ifFailure) =>//<T, U>( Result<T>, ifOk: (value: T) => U, ifFailure: (message: string) => U): U
...
export const mapResult = (f, list) =>//f: T=>Result<U> and list: T[] return a Result<U[]> 
    isEmpty(list) ? makeOk([]) :
    bind(f(first(list)), 
...
export const zipWithResult = (f, xs, ys) =>
